/**
 * Code was pulled and mutated from a demo to fit our needs by James Tully.
 ...
 */
{
    ...
    var slideshow = new Slideshow(document.querySelector('.content'));

    // Preload all the images in the page.
    imagesLoaded(document.querySelectorAll('.content__img-inner'), {background: true}, () => document.body.classList.remove('loading'));

    var timeoutCounter = setTimeout(function(){slideshowNav('next')}, 5000);
    function slideshowNav(direction) {
        clearTimeout(timeoutCounter);
        slideshow.navigate(direction);
        timeoutCounter = setTimeout(function(){slideshowNav(direction)}, 10000);
    }
    document.getElementById("nextButton").addEventListener('click', function(){slideshowNav('next')});
    document.getElementById("prevButton").addEventListener('click', function(){slideshowNav('prev')});
    
    function toggleIn(elem) {
        document.getElementById('demoVideo').pause();
		var list = document.getElementsByClassName("werds"); 
